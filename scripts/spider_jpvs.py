...
def _normalize_spaces(text):
    ...
    text = text.replace("　", " ")  #全角 space to 半角
...
def clean_html_cell(cell_tag,replacement):
    """
    Clean the HTML cell string, removing br label, newlines, and continuous spaces.
    """
    if not isinstance(replacement, str):
        raise TypeError(f"replacement must be str, but it is {type(replacement)}")
    if not cell_tag:
        return ""
    ...
    for br in br_tags:
        br.replace_with("")
    text_content = cell_tag.text.strip()
    text_content = text_content.replace("\n", replacement).replace("\r", replacement)

    return _normalize_spaces(text_content.strip())
